...
void setup() {
  ...
  pinMode(hack1_pin, OUTPUT); //              - Active Low ( GND = ON )
  pinMode(hack2_pin, OUTPUT); //              - Active Low ( GND = ON )
  pinMode(hack3_pin, OUTPUT); //  GUN         - Active Low ( GND = ON )
  pinMode(hack4_pin, OUTPUT); //              - Active Low ( GND = ON )
  pinMode(hack5_pin, OUTPUT); //  RUN         - Active Low ( GND = ON )
  pinMode(12, OUTPUT);        // Data out to main board

  digitalWrite(hack1_pin, LOW); // Needs to be low at beginning
  digitalWrite(hack2_pin, HIGH);
  digitalWrite(hack3_pin, HIGH);
  digitalWrite(hack4_pin, HIGH);

  //  delay(50);
  digitalWrite(hack5_pin, LOW); // BOOT UP RUN SOUND
  delay(200);
...
// Fail safe loop - to force relay off if over 3 hits
